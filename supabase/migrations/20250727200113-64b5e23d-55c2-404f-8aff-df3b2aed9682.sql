...
-- Create strategies table (user_id nullable for predefined strategies)
...
-- Insert some predefined strategies (user_id is NULL for system strategies)
INSERT INTO public.strategies (user_id, name, description, config, is_predefined) VALUES
(NULL, 'Moving Average Crossover', 'Simple moving average crossover strategy', '{"short_ma": 10, "long_ma": 20, "symbol": "NIFTY"}', true),
(NULL, 'RSI Mean Reversion', 'RSI-based mean reversion strategy', '{"rsi_period": 14, "oversold": 30, "overbought": 70}', true),
(NULL, 'Breakout Strategy', 'Price breakout strategy with volume confirmation', '{"lookback_period": 20, "volume_multiplier": 1.5}', true),
(NULL, 'Bollinger Band Squeeze', 'Trading Bollinger Band squeeze setups', '{"period": 20, "std_dev": 2, "squeeze_threshold": 0.1}', true);
