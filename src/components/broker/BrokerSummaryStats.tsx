...
const BrokerSummaryStats: React.FC<BrokerSummaryStatsProps> = ({
  ...
  const getHealthIcon = (score: number) => {
    if (score >= 80) return <CheckCircle className="h-3.5 w-3.5 text-success" strokeWidth={1.75} />;
    if (score >= 60) return <Clock className="h-3.5 w-3.5 text-warning" strokeWidth={1.75} />;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <Card className="glass-panel">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <Wifi className="h-3.5 w-3.5 text-muted-foreground" strokeWidth={1.75} />
        </CardHeader>
        <CardContent>
      ...
      <Card className="glass-panel">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <DollarSign className="h-3.5 w-3.5 text-muted-foreground" strokeWidth={1.75} />
        </CardHeader>
        <CardContent>
